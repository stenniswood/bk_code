...
int* parse_position( char* response_line )
{
	char* ptr = strchr(response_line, ':');
	static int vals[5] = { 0,0,0,0,0 };
	return vals;
}

int* DriveFive::read_position(  )
{
	char cmd[] = "read position";
	send_command( cmd );
	
	bool ok   = read_response();
	int* vals = parse_position( m_response );
	return 	vals;
}

char*  DriveFive::get_device_name(  )
{
	char cmd[] = "get name\r\n";
	send_command( cmd );
	read_response();
	char* colon = strchr( m_response, ':')+2;
	char* eol   = strchr( colon, '\r' );
	*eol = 0;
	strcpy( m_response, colon );
...
/* port must be open() first.
 We can read non drive five boards - just an open file handle
 as long as they have the same baud rate.  All Beyond Kinetics boards
 will respond to "device type" command.		*/
char*	DriveFive::get_device_type	(  )
{
	static char type[50];
	
	send_command("device type");
	read_response();
	
	// split at :  extract after that.
	char* comma = strchr(m_response, ':')+1;
	char* eol   = strchr(m_response, '\r');		
	*eol = 0;
	strcpy( type, comma );
	remove_leading_whitespace( type );
	return type;	
}
...
bool DriveFive::read_response() 
{
	...
	ptr = strchr(m_response, '\n' ) + 1;
	strcpy(m_response, ptr );
