...
void glPhysicalWorld::erase_scene( )
{
    list<glObject*>::iterator iter = m_moveables.begin();
    while (iter != m_moveables.end())
    {
    ...
    m_moveables.erase( m_moveables.begin(), m_moveables.end() );
}
...
void glPhysicalWorld::setup_perspective_matrix(float width, float height)
{
	// Projection matrix : 45° Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
	ProjectionMatrix = glm::perspective(glm::radians(45.0f), width / height, 0.1f, 1000.0f);	
}

void glPhysicalWorld::look_at( GLdouble eyeX, GLdouble eyeY, GLdouble eyeZ,
								GLdouble centerX, GLdouble centerY, GLdouble centerZ,
								GLdouble upX,  	  GLdouble upY, 	GLdouble upZ  )
{
	// Camera matrix : 
	glm::vec3 Camera( eyeX, eyeY, eyeZ );
	glm::vec3 Center( centerX, centerY, centerZ );
	glm::vec3 head_up( 0.0, 1.0, 0.0 );
	ViewMatrix = glm::lookAt( Camera,   // Camera is at (4,3,3), in World Space
