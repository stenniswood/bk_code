...
PanTilt::PanTilt( const char* mDeviceName )
{
	strcpy (m_port_name, mDeviceName );
}

PanTilt::PanTilt( const char* mDeviceName, uint32_t time_out )
{
	strcpy (m_port_name, mDeviceName );	
	timeout = time_out;
}
...
void PanTilt::set_device_name	( const char* mDeviceName )
{
	if (mDeviceName)
		strcpy (m_port_name, mDeviceName );
}
...
void PanTilt::set_baud(int speed)
{
...
void PanTilt::open( const char* mDeviceName )
{
	if (mDeviceName)
		strcpy(m_port_name, mDeviceName );
...
*/
bool PanTilt::send_command(char* mFiveCommand) 
{
...
bool PanTilt::contains_NAK( )
{
	bool isNAK = strstr(m_response, "NAK:") != NULL;	
	return isNAK;
}

bool PanTilt::replace_ctrl_characters( char* mStr, char mReplacement ) 
{
