...
void LoadCell_SerialInterface::Initialize()
{
	last_pos = accum_buff;
	//foot_view = NULL;
...
void LoadCell_SerialInterface::dump_data(unsigned char * b, int count) {
...
/*	if (left_foot)
	...
	if (ptr && ptr_end && logging) 
	{
		...
		if (number>0) sensor_v[0] = atof( sensors[0] );
		if (number>1) sensor_v[1] = atof( sensors[1] );
		if (number>2) sensor_v[2] = atof( sensors[2] );
		if (number>3) sensor_v[3] = atof( sensors[3] );

		/*printf("LoadCells: chars rxd count=%d splits=%d,  ", count, number );
		for (int j=0; j<std::min(number,4); j++)
		{	printf("%6.1f;  ", sensor_v[j] );		}
		printf("\n"); */
		...
		sql_data.sensor[si+2] = sensor_v[2];
		...
		//foot_view->set_sensor_value( 0, sensor_v[0] );
		//foot_view->set_sensor_value( 1, sensor_v[1] );
		//foot_view->set_sensor_value( 2, sensor_v[2] );
		//foot_view->set_sensor_value( 3, sensor_v[3] );
		//foot_view->Invalidate();
		delete( sensors );
	}
}
...
void LoadCell_SerialInterface::set_baud_divisor(int speed)
{
...
int LoadCell_SerialInterface::get_baud(int baud)
{
	switch (baud) {
	case 9600:
		return B9600;
...
void LoadCell_SerialInterface::process_options(int argc, char * argv[])
{
	for (;;) {
		int option_index = 0;
...
void LoadCell_SerialInterface::dump_serial_port_stats()
{
...
void LoadCell_SerialInterface::process_read_data()
{
	unsigned char rb[1024];
	memset(rb, 0, 1024);
	int c = read(_fd, &rb, sizeof(rb));		// Non blocking.
	if (c > 0) {
		if (_cl_rx_dump) {
...
void LoadCell_SerialInterface::process_write_data()
{
	ssize_t count = 0;
...
void LoadCell_SerialInterface::setup_serial_port(int baud)
{
...
int LoadCell_SerialInterface::diff_ms(const struct timespec *t1, const struct timespec *t2)
{
...
int LoadCell_SerialInterface::serial_loadcell_main(int argc, char * argv[])
{
	...
	_write_data = (uint8_t*)malloc(_write_size);
	if (_write_data == NULL) {
