...
{
	static GridPt*  gp = new GridPt();
	gp->row = (byte) rand()%10;
	gp->col = (byte) rand()%10;
	if ((hbm->board[gp->row][gp->col] & 0x0F)>0)		// if already guessed
	{
		gp->row = (byte) rand()%10;
		gp->col = (byte) rand()%10;
	}	
...
GridPt  AI_Player::GuessAfter1Hit(GridPt mHit, BoardMemory* hbm)
{
	...
	while ((!valid) && (possible<0xFF))
	{
		...
		else if ((hbm->board[g->row][g->col]&0x0F)>0) valid = false;
	}
	return *g;
}
...
float AI_Player::findHighestScore(GridPt pts[], int num_grids, float scores[] )
{
	float max_score = 0;
