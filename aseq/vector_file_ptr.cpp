...
**** LIST ROUTINES 
************************************************************/
struct sVectorList** VectorLists = NULL;

void init_lists()
{
	VectorLists = (struct sVectorList**)malloc( Appendages.size() * sizeof(struct sVectorList*) );
	for (int i=0; i<Appendages.size(); i++)
	{
		init_list(i, Appendages[i].actuators.size() );
	}
}
...
struct sOneVector* list_append_vector( struct sVectorList* mlist, struct sOneVector* mv )
{
	if ((mv) && (mlist))
	{
...
{
	struct sOneVector* v = (struct sOneVector*)malloc( sizeof(struct sOneVector) );
	if (v)
	{
		if (DataType==FLOAT)
			v->values.fData = (float*)malloc( sizeof(float)*Dimension );
		else
			v->values.iData = (int*)malloc( sizeof(int)*Dimension );
	}
	return v;
}
...
//	int*  line 	 = atoi_array( char** string, int num 	 );
	...
	struct sOneVector* v = new_vector( Appendages[limb_index].actuators.size(), mDataType );
