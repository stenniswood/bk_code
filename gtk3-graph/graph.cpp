...
void Graph::scroll_new_data( int mSeriesIndex, struct stDataPoint mNewDataPoint )
{
	...
	series_data[mSeriesIndex].data[series_data[mSeriesIndex].data_length] = mNewDataPoint;
...
void  Graph::add_data_series( struct stSeriesInfo& mSeries )
{
	...
	// Auto select color :
	int p = (p_index % sizeof(pallette));
	if (p == theme->skip_pallette_index)  p_index++;
	p = (p_index % sizeof(pallette));
	
	
	printf("palette colors = %d", sizeof(pallette) );	
