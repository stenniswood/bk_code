/* This thread (duplicated form serverthread.c) receives incoming video.
	Done asynchronously and either shows on screen, stores into wave file,
	monitors with machine vision, or reroutes to another device (call forwarding)

Note:  This is not language processing.  The decision of what to do has already been 
		achieved.

Seriously look into already streaming software though!  Such as VLC (receive?).
Maybe this thread establishes the port or something.  (maybe that can be done on the main thread).
    ...
    while(1)
    {
		...
		while (!done)
		{
			...
			else 	// DATA ARRIVED, HANDLE:
			{
				...
				Parse_BASIC_Acknowledgement(header);
