...
byte extract_msg(struct sCAN* Msg, byte* DataPack)
{
	...
	Msg->id.group.id	   = (DataPack[o]) | (DataPack[o+1]<<8);
	Msg->id.group.instance = DataPack[o+2];
	Msg->header.DLC = (DataPack[o+3] & 0x0F);
	Msg->header.rtr = ((DataPack[o+3] & 0xF0)>>4);
	for (int i=0; i<Msg->header.DLC; i++)
	  Msg->data[i] = DataPack[i+o+4];
