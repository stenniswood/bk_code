...
#define MAX_DC_MOTORS 10		// 2 DriveFive boards

struct stDCMotorData
{
...
struct stDCMotorDiagnosticData 
{
	int   status;
	float temperature;
	float current;	
...
struct sequencer_ipc_memory_map
{
	...
	struct stDCMotorData MotorArray[MAX_DC_MOTORS];		// String array (dimension of NumberClients)

	// DIAGNOSTIC DATA :
	long int DiagnosticUpdateCounter;			// Incremented on change to any of below:
	long int DiagnosticAcknowledgedCounter; 	
	struct stDCMotorDiagnosticData DiagnosticArray[MAX_DC_MOTORS];		// String array (dimension of NumberClients)

	// ENABLES :
	long int EnableUpdateCounter;				// Incremented on change to any of below:
	long int EnableAcknowledgedCounter;
	BOOL	 EnableArray[MAX_DC_MOTORS];		// String array (dimension of NumberClients)
