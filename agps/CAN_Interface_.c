...
void CAN_isr()
{
	...
	byte status = read_register( CANSTAT ) & 0x0E;
	switch (status)
	{
	case FLAG_RXB0_INT: 
			// clear interrupt flag - Done Automatically by using SPI_READ_RX cmd 
			get_message( &rxmessage, 0 );
			if (msg_callbacks( &rxmessage) == FALSE)
			{
				printf("Rx0:");
	...
	case FLAG_RXB1_INT:
			...
			break;
	case FLAG_TXB0_INT: bit_modify( CANINTF, 0x04, 0x00 );
			//printf("Cleared TXB0 INTF\n");
			SendNext( );
			break;
	case FLAG_TXB1_INT: bit_modify( CANINTF, 0x08, 0x00 );
			//printf("Cleared TXB1 INTF\n");
			SendNext( );
			break;
	case FLAG_TXB2_INT: bit_modify( CANINTF, 0x10, 0x00 );	
			printf("Cleared TXB2 INTF\n");
			SendNext( );
			break;
	default : printf("CAN Status=%d\n", status); return ;
	}	
	//printf("<<<<<<<------eoi---------\n");
} 
...
{
	printf(" ID=%2X:%4X:%X; ", msg->id.group.block, 
