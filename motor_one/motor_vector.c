...
{
	for (int VectorIndex=0; VectorIndex<mSize; VectorIndex++ )
	{
		pack_move_to_angle( &msg1, mInstances[VectorIndex], mAngles[VectorIndex], mSpeeds[VectorIndex]*100 );
		print_message( &msg1 );
		AddToSendList( &msg1 );
	}
}

void find_vector( byte mInstance )
{
	// for each defined vector
	// scan all instances for a match
	
}

void update_vector( struct sCAN* mMsg  )
{
	// Extract Instance
	// Find vector it belongs to
	// Update the Angle 
}
