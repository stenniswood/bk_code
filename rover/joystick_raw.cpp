...
pthread_t joyThread_id=0;

void JoystickRaw::open( const char* mPathname )
{
	if (mPathname != NULL)
		strcpy( full_device_name, mPathname );
 
     printf("Opening hidraw device: %s\t", full_device_name );
     joystick_fd = ::open( full_device_name, O_RDWR );
     if (joystick_fd <= 0) {
     	char str[80];
		sprintf ( str, "ERROR:  cannot open device %s.  ", full_device_name );
		perror  ( str );
		exit(1);
	 } else
	 	printf("opened!\n");
	...
	int result = pthread_create( &joyThread_id, NULL, &eventThread, this );
	if (result != 0) 
	{
