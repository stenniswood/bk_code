...
{
	file_i2c = open("/dev/i2c-1", O_RDWR); //Open the I2C device file
	if (file_i2c < 0) { //Catch errors
		std::cout << "ERR (i2c_swtiching.cpp:initialize_RPI_servo_hat()): Failed to open /dev/i2c-1. Please check that I2C is enabled with raspi-config\n"; //Print error message
	}
	printf("i2c-1 opened!\n");
...
//  file_i2c = mRobot.device.f_dev;
  ...
  if (status < 0) {
     std::cout << "ERR (i2c_switching.cpp:Initialize_RPI_servo_hat()): Could not get I2C bus with " << addr << " address. Please confirm that ";
  }                                    //  transactions
