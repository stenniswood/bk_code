...
/*	id.group.instance = mInstance;
	...
	byte   id_high     = hi(mIdentifier);
	id.id_array[0] 	   = ((mBlock&0x1F)<<3) | ((id_high&0xE0)>>5);	// CANIDT1
	id.id_array[1] 	   = ((id_high & 0x1C)<<3) | (id_high&0x03);	// CANIDT2
	id.id_array[2] 	   = lo(mIdentifier);							// CANIDT3
	id.id_array[3] 	   = mInstance;									// CANIDT4
    /////////////////////////////////////////////////
	parse_CAN_eid_b( &id, id.id_array );    // fill id portion of the structure for printing.
	return id;
}

/*
INPUT:
	mDestAddress	:	11 bits
	mSourceAddress	:	11 bits
	tos				:	[0=ISO; 1=OEM Defined; 2=Network Managemnt; 3=ISO15765-3 defined]
RETURN:
	tID				:
...
{
	static tID id;
/*	id.group.instance = mDestAddress & 0x00FF;
	id.group.id		  = ((mDestAddress & 0x0700)>>8);
	id.group.id       = (mSourceAddress & 0x00FF)<<3;
	id.group.id       = (mSourceAddress & 0x0700)<<3;
	
	id.diag.priority      = 6;
	id.diag.ext_datapage  = 3;
	id.diag.tos 		  = tos&0x03;
	id.diag.SourceAddress = mSourceAddress;
	id.diag.DestAddress   = mDestAddress;
	return id;
}*/
...
tID  convert_to_id ( char* mtext )
{
	...
	// parse the ':'s	
	char* delim1 = strchr(mtext, ':');
	*delim1 = 0;

	char* delim2 = strchr(delim1+1, ':');
	*delim2 = 0;
	
	id.group.block    = atoi(mtext);
	id.group.id    	  = atoi(delim1+1);
	id.group.instance = atoi(delim2+1);
	return id;		
}
//////////////////////////////////////////////////////	
/*****************
...
void parse_CAN_id( tID* id, byte* marray	)
{
	byte upper = (marray[0] & 0xE0)>>5;
	byte lower = (marray[0] & 0x1F);
	lower |= (marray[1] & 0xE0)>>5;
	
	id->group.id = ((upper<<8) | lower);
}
