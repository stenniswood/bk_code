...
{
	// HEADER:
	int length = 5 + Msg->header.DLC;				// 
	package_int  ( mBuffer, 0, CMD_CAN_MSG );		// Token 
	package_int  ( mBuffer, 4, length );			// Length 
	
	// BODY :
	mBuffer[8] = Msg->id.group.block;				// 
	package_short( mBuffer, 9, Msg->id.group.id );	// 
	mBuffer[11]= Msg->id.group.instance;			// 
	
	mBuffer[12]= (Msg->header.DLC) | ((Msg->header.rtr)<<4);
	for (int i=0; i<Msg->header.DLC; i++)			//
		mBuffer[i+13] = Msg->data[i];				
	return (13+length);
}
