...
void get_acceltime_update()
{
	// update Time between messages:
	gettimeofday( &accel_end, NULL );
	long seconds  = accel_end.tv_sec  - accel_start.tv_sec;
	long useconds = accel_end.tv_usec - accel_start.tv_usec;
	accel_time_delta_prev = accel_time_delta;
	accel_time_delta 	    = seconds + ((float)useconds/1000000.0);
	//printf("*** ACCEL DELTA TIME=%10.4f\n", accel_time_delta);
...
void process_accel( BOOL ShowData )
{
	...
	float mult = 1.0/One_g_scale2g;
	//printf("mult=%6.6f\n", mult);

	scale( &RawxyzAccel, mult, &AccelScaled );
	if (compare_to_1g(&AccelScaled, 3.0*Gravity_variance)!=0)
	{
