...
{
    bool   retval = (ipc_memory_eyes->ResponseCounter > ipc_memory_eyes->ResponseAcknowledgedCounter);
    return retval;
}
...
void  eyes_write_server_event( std::string mCommand )
{
	...
	int MaxAllowedLength = sizeof(ipc_memory_eyes->Response);
	if (length>MaxAllowedLength)
		length = MaxAllowedLength;
		
	ipc_memory_eyes->ResponseCounter++;

	strcpy(ipc_memory_eyes->Response, mSentence);
	printf("|%s|\n", ipc_memory_eyes->Response );
}

void  eyes_acknowledge_server   ( )
{
    ipc_memory_eyes->ResponseAcknowledgedCounter++;
}
void  eyes_wait_for_server      ( )
{
    bool   responded = eyes_new_response_available();
    while( !responded )
        responded = eyes_new_response_available();
}
