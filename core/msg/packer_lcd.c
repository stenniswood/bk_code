...
*/
void pack_lcd_select_font( struct sCAN* mMsg, byte mDestInstance, byte mFont )		
{
	mMsg->id = create_CAN_eid( ID_LCD_COMMAND, mDestInstance );
	mMsg->header.DLC = 2;
	mMsg->header.rtr = 0;	
	mMsg->data[0] = LCD_CMD_SELECT_FONT;
	mMsg->data[1] = mFont; 
	// Not Implemented Yet!
}
...
void pack_lcd_draw_line(struct sCAN* mMsg, byte mDestInstance, byte x1, byte y1, byte x2, byte y2 )
{
	mMsg->id = create_CAN_eid( ID_LCD_COMMAND, mDestInstance );
	mMsg->header.DLC = 5;
	mMsg->header.rtr = 0;	
...
void pack_lcd_draw_arc(struct sCAN* mMsg, byte mDestInstance, byte CenterX, byte CenterY, byte Radius, byte StartAngle, byte StopAngle )
{
	mMsg->id = create_CAN_eid( ID_LCD_COMMAND, mDestInstance );
	mMsg->header.DLC = 6;
	mMsg->header.rtr = 0;	
	...
	mMsg->data[2] = CenterY;
	mMsg->data[3] = Radius;
	mMsg->data[4] = StartAngle;		
	mMsg->data[5] = StopAngle;		
	// not implemented in controller yet.
}
void pack_lcd_draw_circle(struct sCAN* mMsg, byte mDestInstance, byte CenterX, byte CenterY, byte Radius )
{
	mMsg->id = create_CAN_eid( ID_LCD_COMMAND, mDestInstance );
	mMsg->header.DLC = 4;
	mMsg->header.rtr = 0;	
...
void pack_lcd_set_window(struct sCAN* mMsg, byte mDestInstance, byte x1, byte y1, byte x2, byte y2 )
{
	mMsg->id = create_CAN_eid( ID_LCD_COMMAND, mDestInstance );
	mMsg->header.DLC = 5;
	mMsg->header.rtr = 0;	
...
{
	mMsg->id = create_CAN_eid_b( 0, ID_LCD_COMMAND, mDestInstance );
	//printf("mMsg=%x; mMsg.id=%x;  id=%d \n", mMsg, mMsg->id.group.instance, mDestInstance );
